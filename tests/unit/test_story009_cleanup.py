"""Tests for STORY-009: Remove OpenCode/Classic code from pactkit-codex."""
...
class TestAC1NoOpenCodeClassicInDeployer:
    ...
    def test_no_deploy_classic(self):
        import pactkit.generators.deployer as d
...
class TestAC3CLICodexOnly:
    ...
    def test_no_classic_in_choices(self):
        ...
        cli_src = Path("src/pactkit/cli.py").read_text()
...
class TestAC5DeployerSmaller:
    ...
    def test_deployer_under_1000_lines(self):
        from pathlib import Path
        lines = Path("src/pactkit/generators/deployer.py").read_text().count("\n")
